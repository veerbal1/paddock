...
5. **At-any-cost rule (Halter Senior track):** every loop must leave a *hiring artifact*, not just working code. If a loop has no artifact a hiring manager can open in 3 minutes, the loop isn't done.

---

## Target: Halter Senior Engineer — what "good enough" means

You're not proving you can code. You're proving five senior signals from their JD. Each has an owner loop and a concrete artifact:

| Halter JD signal | Where you prove it | Artifact they open |
|---|---|---|
| Turning thousands of collars + billions of events into one calm decision | Loops 4, 5, 7 | `LOAD.md` + reconciled `make demo` scorecard + Grafana breach-latency dashboard |
| Modelling the physical world accurately enough to act in real time | Loops 0, 1, 2, 5 | cow model with cue compliance + hysteresis numbers (60 → 1) + `ST_DWithin` queries with `EXPLAIN` |
| Mobile/web that holds up at 5am in a paddock in the rain | Loop 6 (+ field test) | phone-filmed demo: PWA console on 3G-throttled mobile, offline banner, fence redraw from the field |
| Platforms/pipelines that scale without seams showing | Loops 7, 8, 9 | K8s manifests + HPA + rolling-deploy drill evidence + Terraform + CD pipeline |
| Constrained hardware, flaky connectivity, messy physical world — ship anyway | Loops 3, 4, 9, 10 | retained/LWT/buffered-replay design + byte-budget table (JSON vs protobuf) + mTLS revocation drill |
| Product instinct (cares about the problem, not just the code) | Loops 2, 6, 10 | farm visit report (`FARM.md`) + cue-compliance report + "what I'd do differently" README |
| Ships end-to-end, makes architectural calls and backs them with code | Every loop | `docs/adr/` (one per loop, with rejected options) |
| Lifts the bar, mentors, uses AI tools well | Threads (below) | `CLAUDE.md` + AI-review log + one upstream contribution + onboarding README |
| Owns reliability, on-call | Loops 7, 8 | `RUNBOOK.md` + SLO + game-day postmortem in `NOTES.md` |
| Cloud (AWS bonus), Terraform at scale | Loop 9 | live link + `terraform apply/destroy` + cost note ($/month, what you'd cut) |

**Senior bar for this repo:** a Halter interviewer should be able to (a) open the live link on their phone, redraw a fence, get a Telegram alert, (b) open one ADR and disagree with you intelligently, (c) open `LOAD.md` and see a bottleneck you found with numbers, and (d) open `FARM.md` and see you noticed something your simulator got wrong. If those four work, you're past the resume screen.
...
**Definition of Done:** a stranger with the README can register, *draw a fence on the map with their mouse*, watch cows react to it live, and receive a Telegram message with a Google Maps link when one escapes. Senior additions (non-optional): the console is a mobile-first PWA (installable, usable on a phone over throttled 3G, reconnect + stale-data banner, fence edit queued offline); one real non-engineer has run it from the README in <15 min and their confusion is logged in `FARM.md` v2; per-collar ACLs are proven by a test where collar A replays collar B's topic and the broker refuses.

**Explain test:** What happens to your WebSocket handler when a client on a slow connection stops reading? How does per-device authentication change what a leaked credential can do? Demo it on your phone, on throttled data, with one bar — what breaks first?
...
**Definition of Done:** a dashboard screenshot showing the system holding N-thousand cows with p99 breach-latency under a target *you chose and can defend*; a `LOAD.md` writing up the bottleneck you found and the fix, with before/after numbers. Senior additions: `RUNBOOK.md` v1 exists (SLOs, alert → action table, rollback command, who gets paged for consumer-lag vs no-pings vs mismatch-rate); one game day completed (kill the broker mid-run, follow the runbook, paste the postmortem into `NOTES.md`); the `oncall-assistant` AI tool drafts the incident summary from trace + deploys + logs, with one logged example of where you overrode it.

**Explain test:** What's your breach-detection SLO and why? Show the trace of one alert. What did pprof reveal? Walk me through your runbook for "no pings for 5 min" at 3am — what do you check first, what do you *not* do?
...
**Definition of Done:** fresh clone → `terraform apply` + one pipeline run → live system on a real domain, collars authenticating over TLS from your laptop; teardown is one command. The payoff: you send anyone a link, they open the map on their phone and watch your simulated herd grazing in real time — and you redraw a fence from your own phone and they see the cows react. That link goes in the email, your resume, and your LinkedIn. Senior additions: `COST.md` (monthly $ at current scale, $ at 10× herds, what you'd cut first); revocation drill proven (revoke one collar cert, others unaffected, reconnect attempt refused in broker logs); second game day (roll a bad backend version via CD, roll back via pipeline, time it); if budget allows, the AWS IoT Core comparison note (managed certs/policies/rules-engine vs self-hosted) with one real collar pointed at it.

**Explain test:** Where is your Terraform state and why does it matter? What can an attacker do with one stolen collar credential, and what's your blast radius? What does this cost per month, and what breaks first at 10× scale — and what would you buy vs build then?

---

## Loop 10 — Breadth, polish, and the email (2–3 weeks: 10A tech + 10B job sprint)

**Goal:** convert the system into career artifacts. Split it explicitly: **10A** is technical breadth (Kafka, protobuf, operator), **10B** is a full-time job hunt (resume, video, outreach, mocks). Do not compress 10B into a weekend — applying is the project now.
...
- **Farm field study (v3, mandatory):** you already wrote `FARM.md` v1 in Loop 2 and v2 in Loop 6. Now close it: visit one or two dairy farms near you, ask how they actually track, contain, and move animals, then write half a page on what your simulation got wrong and what you changed. "Get out on farm" is written into nearly every Halter JD; doing it three times, from one of the world's biggest dairy regions, is a signal no other applicant will have.
- **The artifacts (all four, no skipping):** an architecture diagram + honest README (including "what I'd do differently" + byte-budget + cost + SLO); a 2–3 minute demo video **filmed on a phone** (map, live cows, fence redraw from the field, breach, Telegram ping, Grafana); a blog series of three (not one) from `NOTES.md` — the crash-recovery test, the pprof hunt, and the hysteresis 60→1 story; one merged upstream PR (orb / NATS docs / Halter's `valkey-cluster-operator` or `terraform-provider-utils`).
- **10B — The job sprint (1 full week, treat as a loop with its own DoD):** (a) resume: one page, one line per loop with numbers (10k collars load-tested, p99 < 2 s, 60→1 alerts, $X/mo infra); (b) cover letter they explicitly ask for: why Halter + why virtual fencing + what the farm visits taught you, with the live link in paragraph one; (c) apply to the **Senior** listing *and* the evergreen Product & Engineering listing; (d) LinkedIn outreach to engineers/EMs on Guidance, Platform Communications, and Platform teams — short note + live link + 2-min video, never the jobs inbox alone (a working demo gets forwarded internally, and that's the fast path); (e) two mock interviews (one system design: "design virtual fencing for 100k collars"; one behavioral: scars, on-call, mentoring, AI leverage) recorded and reviewed. DoD: 5 tailored outreach messages sent, 2 mocks done, link + video + `LOAD.md` + ADR linked everywhere.
- **The email:** short. "I spent four months building a simulation of a collar-to-cloud virtual fencing pipeline to understand your problem space — edge-side fence enforcement, MQTT with buffered replay, JetStream, PostGIS, K8s — a small version of your half-billion-messages-a-day problem. Demo video and writeup here. I'd love to talk about the real thing." Frame it as studying *their* problem, not cloning their product, and name the teams your project maps to (Guidance, Platform Communications, Platform). It also works as a flagship project for any Go/infra role, not only Halter.

**Explain test:** When would you choose Kafka over JetStream for this system, and what would it cost you operationally? What did you learn on a real farm that your simulation had wrong? What would you do differently if you had a second engineer and a designer for a month?
...
5. **"This feels junior for a Senior role."** It isn't the tech, it's the missing artifact. Check the Senior-bar table at the top: which column has no evidence yet — SLO/runbook, cost, byte-budget, farm notes, AI-review log, upstream PR? That gap is the fix, not more features. Seniors get hired on judgment trails (ADRs, postmortems, trade-off tables), not feature count.
...
- **5 → 6:** hands over fast spatial/time-series queries, multi-farm tenancy hardened with RLS, backups with a proven restore, escalation, richer detections, and `FARM.md` v1. Pain: a powerful system nobody but you can log into, see, or get alerts from.
- **6 → 7:** hands over a complete field-grade product — auth, React PWA console with live map and fence drawing (offline queue + stale banner), Valkey-backed feed, outbox-driven Telegram delivery, optimistic locking on fences, per-collar credentials with topic ACLs, plus `FARM.md` v2 from a real usability test. Pain: you *believe* it's reliable but can't measure, prove, or stress any of it.
- **7 → 8:** hands over two honest latency histograms with a defended SLO, traces, `RUNBOOK.md` v1, one game-day postmortem, health endpoints ready for probes, and a gRPC-driven load generator. Pain: Compose can't do rolling deploys, self-healing, or autoscaling, so your reliability story caps out.
...
- **9 → 10:** hands over a live public system with CD, per-collar mTLS device identity, `COST.md`, and revocation + rollback drills. No pain left to cure — Loop 10 just harvests the career value: protobuf byte-budget, Kafka breadth, the operator stretch, upstream PR, three blogs, phone-filmed demo, and the 10B job sprint.
...
- **Visible AI workflow (Senior proof, not checkbox):** "uses AI tools well" is a first-class criterion in every Halter JD (they name Claude, Claude Code, and Cursor), and Platform explicitly hires for "AI tooling that's reshaping how engineers work." Keep a `CLAUDE.md` in the repo, use AI tools deliberately, and note in the README where they accelerated you *and where you didn't trust them and verified by hand*. At-any-cost additions: (a) log one AI code-review per loop (paste the critique + what you changed) in `docs/ai-reviews/`; (b) in Loop 7–8, ship one small internal AI tool — e.g. a Claude-backed `oncall-assistant` that summarizes a trace + recent deploys + top logs for an alert — because building AI leverage for others is the Platform-team signal, not just using autocomplete.
- **Farm thread (mandatory, moved earlier):** "Get out on farm" is in the JD, so it stops being a Loop 10 flourish. Loop 2: visit or call one dairy farmer near you, write `FARM.md` v1 (how they track/contain/move animals today, what breaks). Loop 6: v2 — watch a non-engineer use your console, note what confused them. Loop 10: v3 — what your simulator still gets wrong. Three versions = product instinct over time, and Punjab dairy context is a differentiator no NZ applicant has.
- **Field-grade mobile thread:** the JD's "5am in a paddock in the rain" is a test spec: gloves, glare, one bar of signal, old Android. From Loop 6 on: console is a mobile-first PWA (installable, big touch targets, high-contrast alert colors), WebSocket reconnect with backoff + stale-data banner, fence edits queued offline and synced. Prove it with a throttled-3G phone video, not a desktop screen recording.
- **Mentor / bar-raising thread (solo-project fix):** you can't list "mentored engineers" from a solo repo, so manufacture the evidence seniors actually leave: (a) every ADR lists rejected options and who it would affect (firmware team? support? farmer?) — that's design-leadership writing; (b) keep `docs/onboarding.md` current enough that a stranger can run the system in <15 min (Loop 6 DoD tests this with a real human); (c) one upstream contribution before Loop 10 — a fix or docs PR to `paulmach/orb`, NATS docs, or Halter's `valkey-cluster-operator` / `terraform-provider-utils` after reading them. One merged PR beats ten "team player" claims.
- **On-call / reliability thread:** from Loop 7 on, maintain `RUNBOOK.md` (SLOs, dashboards, "if X pages, do Y", rollback command) and run one game day per loop (7, 8, 9): kill something on purpose, follow the runbook, write the postmortem in `NOTES.md`. On-call-culture teams screen for this folder.
